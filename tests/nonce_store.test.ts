...
import { InMemoryNonceStore, SqliteNonceStore, MAX_NONCE_TTL_MS } from '../backend/nonce_store';
...
describe('SqliteNonceStore TTL eviction', () => {
  ...
  it('removes expired rows rather than merely hiding them', async () => {
    ...
    const rows = db.prepare('SELECT nonce FROM x402_used_nonces').all() as { nonce: string }[];
