...
// ── Concurrent execution and instance isolation (#67) ──────────────────────

describe("Concurrent execution and instance isolation", () => {
  it("two PayFiAgent instances do not share state when run concurrently", async () => {
    ...
    expect(mockInstance1.execute).toHaveBeenCalledTimes(1);
    expect(mockInstance2.execute).toHaveBeenCalledTimes(1);
