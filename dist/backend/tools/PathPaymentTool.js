...
exports.PathPaymentInputSchema = zod_1.z.object({
    ...
    allowSelfPayment: zod_1.z.boolean().optional().default(false),
...
class PathPaymentTool {
    ...
    async execute(rawInput) {
        ...
        if (pathAssets.length === 0 && typeof rpc_client_1.horizonServer?.strictSendPaths === 'function') {
            const paths = await rpc_client_1.horizonServer
                .strictSendPaths(sendAsset, input.sendAmount, [destAsset])
                .call();
            if (!paths || !paths.records || paths.records.length === 0) {
                throw new errors_1.ValidationError('No path found between assets');
            }
            const bestRecord = paths.records[0];
            if (bestRecord && Array.isArray(bestRecord.path)) {
                pathAssets = bestRecord.path.map((p) => {
                    if (p.asset_type === 'native' || p.code === 'XLM')
                        return stellar_sdk_1.Asset.native();
                    return new stellar_sdk_1.Asset(p.asset_code || p.code, p.asset_issuer || p.issuer);
                });
            }
        }
