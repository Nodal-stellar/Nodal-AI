...
vitest_2.vi.mock('../../backend/config', () => ({
    config: {
...
function randomOverLimitAmount() {
...
function randomScientificNotation() {
...
function validateAmount(amount) {
    return StellarPaymentTool_1.PaymentInputSchema.safeParse({
...
// ─── Tests ────────────────────────────────────────────────────────────────────
(0, vitest_1.describe)('PaymentInputSchema — amount fuzz (property-based, 500 cases each)', () => {
    (0, vitest_1.it)('rejects all negative amounts', () => {
        const failures = [];
    ...
    (0, vitest_1.it)('rejects all zero / zero-equivalent amounts', () => {
        const failures = [];
        for (let i = 0; i < RUNS; i++) {
    ...
    (0, vitest_1.it)('accepts all valid positive amounts within the spending limit', () => {
        const failures = [];
        for (let i = 0; i < RUNS; i++) {
            const amount = randomValidAmount();
            const result = validateAmount(amount);
            if (!result.success)
                failures.push(`${amount} → ${result.error.issues[0]?.message}`);
        }
        (0, vitest_1.expect)(failures).toHaveLength(0);
    });
    (0, vitest_1.it)('rejects amounts with more than 7 decimal places', () => {
        const failures = [];
        for (let i = 0; i < RUNS; i++) {
    ...
    (0, vitest_1.it)('rejects scientific notation amounts', () => {
        const failures = [];
        for (let i = 0; i < RUNS; i++) {
    ...
    (0, vitest_1.it)('rejects non-numeric / garbage strings', () => {
        const failures = [];
        ...
        for (let i = 0; i < RUNS; i++) {
            ...
            const result = validateAmount(amount);
            if (result.success)
                failures.push(JSON.stringify(amount));
        }
        (0, vitest_1.expect)(failures).toHaveLength(0);
    });
    (0, vitest_1.it)('amounts above AGENT_SPENDING_LIMIT pass schema but are flagged as over-limit', () => {
        ...
        (0, vitest_1.expect)(schemaAccepted).toBeGreaterThan(0);
        // Every schema-accepted over-limit amount must actually exceed the limit
        (0, vitest_1.expect)(wronglyUnderLimit).toHaveLength(0);
        (0, vitest_1.expect)(correctlyOverLimit).toBe(schemaAccepted);
    });
    (0, vitest_1.it)('invariant: safeParse never throws — always returns a result object', () => {
